...
C						!MUNG MESSAGE.
	...
	I=653
C						!NOVELTY 1.
	IF(IAND(OFLAG2(PRSO),FITEBT).NE.0) I=654
...
C						!PUSHED MIRROR WALL.
	RETURN
C
7100	IF(IAND(RFLAG(HERE),RNWALL).EQ.0) GO TO 10
	CALL RSPEAK(662)
C						!NO WALL.
	RETURN
C NOBJS, PAGE 6
...
C
	...
	IF(PNUMB.EQ.4) OFLAG1(ODOOR)=IOR(OFLAG1(ODOOR),VISIBT)
C
...
C						!KILL CARD.
	CPOUTF=.TRUE.
C						!OPEN DOOR.
	OFLAG1(STLDR)=IAND(OFLAG1(STLDR),not(VISIBT))
	RETURN
C
...
C						!DESCRIBE.
	...
	END
C
C Processor for global mirror/panel
