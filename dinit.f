...
D150	FORMAT('RESTORING FROM "dindx.dat"')
	READ(1,*) MXSCOR,EGMXSC
	READ(1,*) RLNT,RDESC1(1:RLNT),RDESC2(1:RLNT),REXIT(1:RLNT),
     &	RACTIO(1:RLNT),RVAL(1:RLNT),RFLAG(1:RLNT)
	READ(1,*) XLNT,TRAVEL(1:XLNT)
	READ(1,*) OLNT,ODESC1(1:OLNT),ODESC2(1:OLNT),ODESCO(1:OLNT),
	...
	READ(1,*) R2LNT,OROOM2(1:R2LNT),RROOM2(1:R2LNT)
	READ(1,*) CLNT,CTICK(1:CLNT),CACTIO(1:CLNT)
	READ(1,*) CFLAG(1:CLNT)
	READ(1,*) VLNT,VILLNS(1:VLNT),VPROB(1:VLNT),VOPPS(1:VLNT),
     &	VBEST(1:VLNT),VMELEE(1:VLNT)
	READ(1,*) ALNT,AROOM(1:ALNT),ASCORE(1:ALNT),AVEHIC(1:ALNT),
     &	AOBJ(1:ALNT),AACTIO(1:ALNT),ASTREN(1:ALNT),AFLAG(1:ALNT)
	READ(1,*) STRBIT,MBASE
	READ(1,*) MLNT,RTEXT(1:MLNT)
C
	CLOSE(1)
	GO TO 1025
C						!INIT DONE.
...
C	CALL INIRND(SHOUR*3600+SMIN*60+SSEC)
	CALL INIRND(XOR(XOR(SHOUR,SMIN),SSEC))
C
	WINNER=PLAYER
	LASTIT=AOBJ(PLAYER)
	HERE=AROOM(WINNER)
